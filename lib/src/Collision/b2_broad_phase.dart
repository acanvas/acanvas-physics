...
//:
	...
	// Create and destroy proxies. These call Flush first.
	  dynamic CreateProxy(b2AABB aabb,dynamic userData){
		...
		for (int axis = 0; axis < 2; ++axis)
		{
			...
			QueryAxis(lowerIndexOut, upperIndexOut, lowerValues[axis].toInt(), upperValues[axis].toInt(), bounds, boundCount, axis);
	  ...
	  void Query(Function callback,b2AABB aabb)
	{
		...
		List<int> upperIndexOut = new List<int>();
		upperIndexOut.add(upperIndex);
		QueryAxis(lowerIndexOut, upperIndexOut, lowerValues[0].toInt(), upperValues[0].toInt(), m_bounds[0], 2*m_proxyCount, 0);
		QueryAxis(lowerIndexOut, upperIndexOut, lowerValues[1].toInt(), upperValues[1].toInt(), m_bounds[1], 2*m_proxyCount, 1);
...
//:
	  void ComputeBounds(List<double> lowerValues,List<double> upperValues,b2AABB aabb)
	{
		...
		lowerValues[0] = ((m_quantizationFactor.x * (minVertexX - m_worldAABB.lowerBound.x)).toInt() & (b2Settings.USHRT_MAX - 1)).toDouble();
		upperValues[0] = (((m_quantizationFactor.x * (maxVertexX - m_worldAABB.lowerBound.x)).toInt() & 0x0000ffff) | 1).toDouble();
		
		lowerValues[1] = ((m_quantizationFactor.y * (minVertexY - m_worldAABB.lowerBound.y)).toInt() & (b2Settings.USHRT_MAX - 1)).toDouble();
		upperValues[1] = (((m_quantizationFactor.y * (maxVertexY - m_worldAABB.lowerBound.y)).toInt() & 0x0000ffff) | 1).toDouble();
	}
	  ...
	  bool TestOverlapBound(b2BoundValues b,b2Proxy p)
	{
	  ...
	  void QueryAxis(List<int> lowerQueryOut,List<int> upperQueryOut,int lowerValue,int upperValue,List<b2Bound> bounds,int boundCount,int axis){
